...
yarn config set proxy http://127.0.0.1:7897 2>nul
yarn config set https-proxy http://127.0.0.1:7897 2>nul
if %errorlevel% equ 0 (
...
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v ProxyServer /t REG_SZ /d "127.0.0.1:7897" /f >nul
echo System proxy set successfully

echo.
echo All proxies have been set successfully!
echo Proxy server: 127.0.0.1:7897
pause